...
def test_ft_filter():
    """
    Test la fonction personnalisée ft_filter pour s'assurer qu'elle
    fonctionne comme prévu.
    """

    # Cas 1 : Test de ft_filter avec une fonction qui retourne
    # True pour les nombres pairs
    ...
    assert result == expected_output, (
        f"Erreur dans ft_filter avec les nombres pairs : {result}")
    print("Cas 1 réussi !\n")

    # Cas 2 : Test de ft_filter avec None (filtre les éléments 'truthy'
    # uniquement)
    ...
    print(f"Résultat obtenu : {result}")
    assert result == expected_output, (
        f"Erreur dans ft_filter avec None comme fonction : {result}")
    print("Cas 2 réussi !\n")
...
def test_filterstring():
    """
    Test le programme filterstring.py pour s'assurer qu'il
    fonctionne correctement dans différents cas.
    """

    # Cas 1 : Test avec une chaîne valide et un entier (filtre
    # les mots de longueur > 4)
    input_string = 'Hello the World'
    input_length = '4'
    print(f"Cas 1 - Input : '{input_string}' et longueur {input_length}")
    result = subprocess.run(['python3', 'filterstring.py',
    ...
    assert result.stdout == expected_output, (
        f"Erreur dans filterstring.py avec '{input_string}' et "
        f"{input_length} : {result.stdout}")
    print("Cas 1 réussi !\n")
    ...
    input_string = 'Hello the World'
    input_length = '3'
    print(f"Cas 3 - Input : longueur '{input_length}' avant la chaîne "
          f"'{input_string}' (ordre incorrect)")
    result = subprocess.run(['python3', 'filterstring.py',
    ...
    print(f"Résultat obtenu : {result.stdout.strip()}")
    assert result.stdout == expected_output, (
        f"Erreur dans filterstring.py avec mauvais ordre "
        f"d'arguments : {result.stdout}")
    print("Cas 3 réussi !\n")
    ...
    print(f"Résultat obtenu : {result.stdout.strip()}")
    assert result.stdout == expected_output, (
        f"Erreur dans filterstring.py sans arguments : {result.stdout}")
    print("Cas 4 réussi !\n")
...
def main():
    ...
    Appelle les fonctions de tests pour ft_filter et filterstring.py.
    """
