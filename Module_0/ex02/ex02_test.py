...
def test_find_ft_type():
    ...
    if not result.stdout:
        raise AssertionError("Erreur : Aucune \
                             sortie obtenue du script tester.py")
    ...
    assert len(lines) == len(expected_output), (
        f"Erreur : nombre de lignes inattendu.\
            Attendu : {len(expected_output)}, Reçu : {len(lines)}"
    )
    ...
    result_find_ft_type = subprocess.run(['python3', 'find_ft_type.py'],
                                         capture_output=True, text=True)

    # Vérifier qu'il n'y a pas de sortie pour find_ft_type.py exécuté seul
    assert result_find_ft_type.stdout == "", (
        "Erreur : find_ft_type.py ne doit pas produire de sortie \
            lorsqu'il est exécuté seul"
    )
