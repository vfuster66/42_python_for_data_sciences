...
def display_graph(data):
    """
    Display the graph for the given data.

    Parameters:
        data (pd.DataFrame): Merged dataset with GNP and life expectancy
    """
...
def main():
    """
    Load and display data for Gross National Product (GNP) per capita
    and life expectancy from the year 1900, with visualization.
    """
    ...
    if year_1900_column not in income_data.columns or \
            year_1900_column not in life_expectancy_data.columns:
        print(f"Erreur : La colonne '{year_1900_column}' n'existe pas dans "
              f"l'un des fichiers.")
        return

    # Sélectionner les données pour l'année 1900
    income_1900 = income_data[['country', year_1900_column]].dropna()
    life_expectancy_1900 = life_expectancy_data[['country',
                                                 year_1900_column]].dropna()

    # Fusionner les deux datasets sur la colonne 'country'
    merged_data = pd.merge(income_1900, life_expectancy_1900,
