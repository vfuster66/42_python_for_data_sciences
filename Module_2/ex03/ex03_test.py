...
def test_load_valid_files():
    ...
    assert income_data is not None, (
        f"Erreur : Le fichier '{income_file}' n'a pas pu être chargé."
    )
    assert life_data is not None, (
        f"Erreur : Le fichier '{life_expectancy_file}' n'a pas pu "
        "être chargé."
    )

    print(
        f"Test réussi : Les fichiers '{income_file}' et "
        f"'{life_expectancy_file}' ont été chargés avec succès."
    )
...
def test_file_not_found():
    ...
    assert df is None, (
        f"Erreur : Le fichier inexistant '{missing_file}' aurait dû "
        "déclencher une erreur."
    )
    print(
        f"Test réussi : Le fichier inexistant '{missing_file}' a "
        "déclenché le bon message d'erreur."
    )
...
def test_missing_column():
    ...
    if income_data is not None:
        ...
        if '1900' not in modified_data.columns:
            print(
                "Test réussi : La colonne '1900' est manquante, comme prévu."
            )
        else:
            print("Erreur : La colonne '1900' n'a pas été correctement "
                  "supprimée.")
    else:
...
def test_graph_generated():
    ...
    assert os.path.exists(graph_file), (
        f"Erreur : Le fichier '{graph_file}' n'a pas été créé."
    )
